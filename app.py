...
def main():
    ...
    max_tenure = float(data['tenure'].max())  # Ensure max_tenure is a float
    max_monthly_charges = float(data['MonthlyCharges'].max())  # Ensure max_monthly_charges is a float
    max_total_charges = float(data['TotalCharges'].max())  # Ensure max_total_charges is a float
    ...
    # If Calculate CHURN Probability option is selected
        ...
        tenure = st.number_input("Tenure:", min_value=0.0, max_value=max_tenure, step=1.0)  # Use float for step
